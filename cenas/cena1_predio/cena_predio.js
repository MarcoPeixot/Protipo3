...
export default class Scene1 extends Phaser.Scene {
	...
	create() {
		...
		this.tecla_E = this.add
			.sprite(this.tyler.x, this.tyler.y - 80, 'movimentacao')
			.setOrigin(0.5, 0.5)
			.setVisible(true)
			.setScale(2);
		...
		Texto.letraPorLetra(this, falas[0], this.textoInicio);
		this.tyler.setDepth(2);
	}

	// Cria o mapa da cena
	criarMapa() {
		// Cria o mapa e define as camadas com colisões
		// Cada camada do mapa pode ser uma camada de tiles pelo Tiled
	...
	update() {
		...
		if (this.tyler.x >= 800 && this.tyler.y <= 450) {
			this.transitionToMainScene('cena_escritorio');
			this.passosConcreto.stop();
