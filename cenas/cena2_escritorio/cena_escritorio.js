...
export default class SceneOffice extends Phaser.Scene {
	...
	preload() {
		this.load.image(
			...
			'./assets/mapas/mapa_escritorio/mapaEscritórioFinal.png'
		);
		this.load.image(
			'escritorio_ground',
			'./assets/mapas/mapa_escritorio/escritorio base(ground).png'
		);
		this.load.tilemapTiledJSON(
			'map_escritorio',
			'./assets/mapas/mapa_escritorio/escritorio.json'
		);
	}

	// Criação dos elementos da cena
	create() {
		console.log(mudarCenaEscritorio)
		...
		this.tecla_E = this.add
			.sprite(this.tyler.x, this.tyler.y - 20, 'tecla_e')
			.setOrigin(0.5, 0.5)
			.setVisible(false)
			.setScale(1);
		...
		this.tecla_sinalizcao = this.add
			.sprite(this.vanessa.x, this.vanessa.y - 30, 'tecla_sinalizacao')
			.setOrigin(0.5, 0.5)
			.setVisible(true)
			.setScale(1.5);
		...
		this.textoTesteResumo = this.add.text(this.tyler.x, this.tyler.y, dialogs[idioma]['texto_resumo1'],{
			fontFamily: 'Arial',
			fontSize: 12,
			color: 'black',
		});
	...
	mostrarProximoDialogo() {
		...
		if (!this.textoEmAndamento && this.mudaDialogo < falas.length) {
			Texto.letraPorLetra(this, falas[this.mudaDialogo], this.textoVanessa);
	...
	mostrarProximoDialogoVolta() {
		...
		if (!this.textoEmAndamento && this.mudaDialogo < falasVolta.length) {
			Texto.letraPorLetra(this, falasVolta[this.mudaDialogo], this.textoVanessa);
			this.mudaDialogo++; // Incrementa o índice para o próximo diálogo
		}
	}
